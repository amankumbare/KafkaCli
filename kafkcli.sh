...
if [ "$SECURITY_PROTOCOL" == "SASL_PLAINTEXT" ]  || [ "$SECURITY_PROTOCOL" == "PLAINTEXTSASL" ] ; then
    echo "yes!"
	while true; do
	...
	if  [ "$KAFKAUSER" == "no" ];then
		...
		kinit -kt $KAFKA_PROCESS_DIR/kafka.keytab $PRINCIPAL
		exit_on_failure
	fi
else
 :
...
options=(
	"LIST ALL TOPIC" "DESCRIBE ALL THE TOPICS" 
